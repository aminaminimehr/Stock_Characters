# alm Character

Description: Quarterly Asset Liquidity

Primary reference in the Green-style summary: Ortiz-Molina and Phillips (2014).

Construction follows the Green SAS reference where the formula is available, while the panel timing follows this repository's `signal_yyyymm` / `target_yyyymm` convention.

Status: Implemented through the shared Green SAS builder.

## Run

```powershell
python Character_Builders/Green_ALM_Generalized/build_alm.py --wrds-user YOUR_WRDS_USERNAME
```
