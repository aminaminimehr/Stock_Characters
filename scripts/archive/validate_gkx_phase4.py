#!/usr/bin/env python3
"""Lightweight validation for GKX Phase 4 annual characteristics."""
...
def main():
    parser = argparse.ArgumentParser(description="Validate GKX Phase 4 batch outputs.")
    ...
    lines.extend(
        [
            ...
            "## Notes",
            "",
            "- Low Pearson with high Spearman often reflects outlier-driven level differences.",
            "- Phase 1–3 variables were not modified in this batch.",
            "- `realestate`, `obklg`, `chobklg` deferred pending methodology audit.",
        ]
    )

    text = "\n".join(lines) + "\n"
    docs_out = PROJECT_ROOT / "docs" / "gkx" / "gkx_phase4_validation.md"
    docs_out.parent.mkdir(parents=True, exist_ok=True)
    docs_out.write_text(text, encoding="utf-8")
    diag_out = DIAGNOSTICS_DIR / "gkx_phase4_validation.md"
